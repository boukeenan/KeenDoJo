...
namespace KeenDoJoWindowsApp
{
	public partial class frmXmlDoc : Form
	{
		...
		private void CreateXmlFile()
		{
			...
			xmlDocCreate = new XDocument(
				...
				new XElement(xmlTags[0],
					new XElement(xmlTags[1],
						...
						, new XElement(xmlTags[11], "CLOCKSETUP"))		// OriginatingPages
						, new XElement(xmlTags[12]
						, new XElement(xmlTags[13], "lblC2_Stuff"))	// ControlNames
						, new XElement(xmlTags[14], 1)					// ReadValue
						, new XElement(xmlTags[15], "Error message")	// Error Message
				)));

			xmlDocCreate.Declaration = new XDeclaration("1.0", "utf-8", "true");
			...
			//KBREAK.Break();
			xmlDocCreate.Save(config.PlcModbusDatabaseLocation);

			lblXmlResults.Text = xmlDocCreate.ToString();
		}

		private void cmdAddToXmlDoc_Click(object sender, EventArgs e)
		{
			xmlDocCreate.Element(xmlTags[0]).Add(
				new XElement(xmlTags[1], 
						new XElement(xmlTags[2], "C3"),                 // Plc Address
						new XElement(xmlTags[3], 16387),                // ModbusAddress
						new XElement(xmlTags[4], "OutputCoils"),        // AddressType
						new XElement(xmlTags[5], "C")                   // AddressPrefix
						, new XElement(xmlTags[6], 16386)               // uiModbusAddress
						, new XElement(xmlTags[7], DateTime.Now)        // InitFresh
						...
						, new XElement(xmlTags[11], "CLOCKSETUP"))      // OriginatingPages
						, new XElement(xmlTags[12]
						, new XElement(xmlTags[13], "lblC3_Stuff")) // ControlNames
						, new XElement(xmlTags[14], 1)                  // ReadValue
						, new XElement(xmlTags[15], "Error message")    // Error Message
				));
